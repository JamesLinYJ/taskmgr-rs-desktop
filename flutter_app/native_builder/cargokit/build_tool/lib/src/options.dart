...
enum Toolchain {
  ...
  const Toolchain(this.rustupName);

  final String rustupName;
}
...
class CargoBuildOptions {
  ...
  static Toolchain _toolchainFromNode(YamlNode node) {
    if (node case YamlScalar(value: String name)) {
      final toolchain =
          Toolchain.values.firstWhereOrNull((element) => element.rustupName == name);
      if (toolchain != null) {
        return toolchain;
      }
    }
    throw SourceSpanException(
        'Unknown toolchain. Must be one of ${Toolchain.values.map((e) => e.rustupName)}.',
        node.span);
  }
